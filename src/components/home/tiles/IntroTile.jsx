import React, { useState, useEffect, useMemo } from "react";
import { profileData } from "@/data/profile";

export default function IntroTile() {
  const [activeBubble, setActiveBubble] = useState(0);
  const [showBubble, setShowBubble] = useState(true);

  const greeting = useMemo(() => {
    const hour = new Date().getHours();
    if (hour < 5) return "Working late 🌙";
    if (hour < 12) return "Good morning ☀️";
    if (hour < 15) return "Good noon 🌤️";
    if (hour < 18) return "Good afternoon 🌞";
    if (hour < 22) return "Good evening 🌆";
    return "Good night 🌃";
  }, []);

  const chatLines = useMemo(
    () => [profileData.chatLines[0], greeting, ...profileData.chatLines.slice(1)],
    [greeting]
  );

  useEffect(() => {
    let revealTimeout = null;

    const rotateBubbles = window.setInterval(() => {
      setShowBubble(false);

      revealTimeout = window.setTimeout(() => {
        setActiveBubble((current) => (current + 1) % chatLines.length);
        setShowBubble(true);
      }, 1800);
    }, 7500);

    return () => {
      window.clearInterval(rotateBubbles);
      if (revealTimeout !== null) {
        window.clearTimeout(revealTimeout);
      }
    };
  }, [chatLines]);

  return (
    <div className="flex flex-col justify-center h-full px-10 lg:px-16 bg-white dark:bg-[#0d1117] rounded-4xl dark:ring-2 dark:ring-gray-700">
      <div className="flex items-center gap-3.5 mb-2">
        {/* Avatar Image - Uncropped full memoji */}
        <div className="relative w-20 h-24 flex items-center justify-center flex-shrink-0 select-none">
          <img
            src={profileData.avatar.memoji}
            alt={`${profileData.name}'s Avatar`}
            fetchPriority="high"
            decoding="sync"
            className="w-full h-full object-contain pointer-events-none"
          />
        </div>

        {/* Bubble container aligned with the avatar */}
        <div className="min-h-[56px] flex items-center" aria-live="polite" aria-label="Greeting chat">
          {showBubble ? (
            <div
              className="relative w-fit max-w-[260px] rounded-[28px] bg-[#0A84FF] px-5 py-3 text-[15px] font-semibold leading-snug text-white shadow-[0_12px_28px_rgba(10,132,255,0.4)] ring-1 ring-[#a5dbff]/30 transition-all duration-500"
            >
              <span className="break-words">{chatLines[activeBubble]}</span>
              <svg
                className="pointer-events-none absolute -left-[9px] top-1/2 -translate-y-1/2 h-[18px] w-[12px]"
                viewBox="0 0 12 18"
                fill="none"
                aria-hidden="true"
              >
                <path
                  d="M12 2C7.5 4 3 6.5 0 9C3 11.5 7.5 14 12 16V2Z"
                  fill="#0A84FF"
                />
              </svg>
            </div>
          ) : (
            <div
              className="flex w-fit items-center gap-1.5 rounded-full bg-[#0A84FF] px-4 py-3 shadow-[0_12px_28px_rgba(10,132,255,0.4)] ring-1 ring-[#a5dbff]/30 transition-all duration-300"
            >
              <span
                className="h-2 w-2 rounded-full bg-white animate-bounce"
                style={{ animationDuration: "1s", animationDelay: "0ms" }}
              />
              <span
                className="h-2 w-2 rounded-full bg-white animate-bounce"
                style={{ animationDuration: "1s", animationDelay: "150ms" }}
              />
              <span
                className="h-2 w-2 rounded-full bg-white animate-bounce"
                style={{ animationDuration: "1s", animationDelay: "300ms" }}
              />
            </div>
          )}
        </div>
      </div>

      <h1 className="text-gray-500 text-base leading-relaxed">
        I'm{" "}
        <span className="relative inline-block whitespace-nowrap z-10 font-bold text-gray-800 dark:text-gray-100">
          <span className="relative z-20 font-decorative text-[20px]">{profileData.name}</span>
          <svg
            viewBox="0 0 977 88"
            fill="none"
            className="absolute left-0 w-full -bottom-1 h-2 pointer-events-none text-[#4122FF] dark:text-[#818cf8]"
            preserveAspectRatio="none"
            aria-hidden="true"
          >
            <path
              d="M50.8204 20.0555C13.3545 46.3605 5.61884 50.4506 0.017324 59.7959C0.0173169 63.5154 -0.373476 64.1027 2.55748 67.8223C26.6872 51.3301 57.8547 28.0818 70.3601 33.9548C82.8654 39.8278 89.1181 90.4659 116.474 86.8116C135.427 84.2797 130.347 68.0181 170.208 37.2828C181.736 68.8011 202.057 71.713 214.563 59.2086C218.478 55.2933 228.215 47.4968 236.056 39.0447C240.399 62.7355 258.527 70.7588 269.078 62.7324C278.668 55.4378 287.25 45.3092 300.537 37.2828C335.318 81.3301 327.893 54.9018 377.328 32.3887C409.959 96.5998 434.579 49.4203 472.682 33.3675C509.612 98.3617 531.301 56.2721 568.817 33.3675C588.943 84.4624 607.114 50.0076 643.263 28.2776C687.813 75.6529 696.02 41.3939 735.294 26.7115C767.144 86.8116 781.408 28.0819 823.614 29.8438C840.222 60.9705 859.846 55.0328 893.761 30.6268C893.761 30.6268 900.662 41.399 909.784 42.9601C923.461 45.301 923.461 39.4363 947.495 28.0819C956.483 23.8355 962.409 20.3062 977 19.0766V5.37296C964.472 7.37539 953.748 6.58772 944.173 12.0291C920.228 25.6378 913.233 20.7002 908.611 13.3994C906.905 10.703 907.382 8.01302 905.876 5.37296C895.715 -12.4414 869.532 19.2723 845.889 24.9495C832.016 -8.33064 794.109 1.65353 759.914 25.9284C759.914 25.9284 750.535 5.55299 739.202 1.65355C719.858 -5.0024 702.077 11.4418 679.216 25.9284C668.85 32.4968 652.642 2.43656 641.504 2.43656C624.7 2.43656 590.506 31.8014 590.506 31.8014C590.506 31.8014 589.972 14.7138 579.173 8.30952C557.484 -4.55325 526.611 34.5421 509.612 41.5897C492.05 48.8703 498.865 19.6412 481.474 8.70105C464.084 -2.23906 418.361 39.0447 406.833 39.8278C399.989 40.2927 396.086 11.4418 382.213 8.30952C359.007 3.07022 323.008 34.5421 323.008 34.5421C323.008 34.5421 314.997 15.1613 300.537 11.4418C283.328 7.01475 258.527 36.4998 258.527 36.4998C258.527 36.4998 261.067 11.4418 244.263 6.54763C234.124 3.59469 193.851 40.8066 193.851 40.8066C193.851 40.8066 186.374 20.1962 173.334 13.2037C157.507 4.71681 124.68 50.2034 110.807 63.7112C102.622 71.6807 88.2863 -6.24959 50.8204 20.0555Z"
              fill="currentColor"
            />
          </svg>
        </span>
        , a {profileData.title} from {profileData.location.city}...
      </h1>
      <p className="mt-1 text-gray-500 text-base leading-relaxed max-w-[500px]">
        {profileData.bio}
      </p>
    </div>
  );
}
